...
var renderLineChart = function (timeseries) {
    ...
    var chartData; // data values stored as number for highcharts
    ...
    var decimalPlaces = 0; // Store number of DPs for use when rendering table. Fixed issue where numbers ending '.0' don't show DP.
    ...
    function renderTable() {
        ...
        for (var i = 0; i < chartData.values.length; i++) {
            var current = chartData.values[i];
            var currentValue = current.y;
            var tr = $(document.createElement('tr')).addClass('table__row');

            // Add data into array to be used for sorting on click of a header
            sortArray.push({
                'date': current.name,
                'value': current.y
            });

            // Check that current value isn't null else fix to correct decimal place
            if (currentValue) {
                currentValue = currentValue.toFixed(decimalPlaces);
            }

            // Empty table and add data
            tbody.append(tr);
            tr.append('<th scope="row" class="table__data text-left">' + current.name + '</th>');
            tr.append('<td class="table__data text-right">' + currentValue + '</td>');
        }
        show(table);
    ...
    function formatData(timeseriesValues) {
        ...
        for (i = 0; i < timeseriesValues.length; i++) {
            ...
            // Store how many decimal places the numbers have - used to fixed '.0' being removed from numbers
            if ((+current.value.toString().length == current.value.length) && current.value.indexOf('.') > 0) {
                decimalPlaces = current.value.split(".")[1].length;
            }
        ...
        function bindDisplayChangeButtons() {
            $('[data-chart-controls-type]', element).on('click', function (e, data) {
                ...
                tableBtn.off().click(function () {
                    ...
                    triggerSort(sortArray, column, currentFrequency, decimalPlaces);
